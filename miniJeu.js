...
// Il faut 3 paramètres (un nombre de départ / un nombre de fin / un nombre d'essai)
...
function getRandomNumber(level) {
    ...
    const randomNumber = Math.floor(Math.random() * (level.endNumber - level.startNumber) + level.startNumber);
...
function run(level) {
    ...
    const playedNumbers = [ 3, 9, 7, 2, 5 ];
    playedNumbers.length = level.attempt;

    playedNumbers.forEach((number, index) => {
        let attempt = index > 0 ? "essais" : "essai";
        if(number === randomNumber) {
            console.log(`Vous êtes le meilleur et vous avez gagné en ${index + 1} ${attempt} essais`);
        } else {
            console.log(`Veuillez retentez votre chance`);
        }
    });
}
...
//console.log(playedNumbers.length);

// Comparer le nombres proposés avec le nombre généré

// Afficher un message avec le nombre d'essais, le numéro mystère, ressayer
